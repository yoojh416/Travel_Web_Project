...
public class BookingController {
    ...
    @GetMapping("/modify") //예약 내용 수정
    public String modifyOrder(@PathVariable("orderId") Long orderId, Model model) {
        Booking booking = bookingRepository.findById(orderId).get();
        model.addAttribute("booking", booking);
        List<Booking> order = bookingRepository.findAll();
        model.addAttribute("order", order);

        return "order/confirmation"; //confirmation이 아닌 modifyForm으로 가야 함
    }
